...
#define S_IWGRP 0
#endif
static inline int mkstemp(char *tmpl) { UNUSED(tmpl); return -1; }
...
bx_hard_drive_c::init(void)
{
  ...
  BX_HD_THIS channels[0].ioaddr1 = 0x01f0;
  BX_HD_THIS channels[0].ioaddr2 = 0x03f0;
  BX_HD_THIS channels[0].irq = 14;
  ...
  for (channel=0; channel<BX_MAX_ATA_CHANNEL; channel++) {
    for (Bit8u device=0; device<2; device ++) {
      ...
      bx_bool dev_present;
      bx_bool dev_is_cdrom;
      if (device == 0) {
        dev_present = bx_options.diskc.Opresent->get() ? 1 : 0;
        dev_is_cdrom = 0;
        }
      else {
        dev_is_cdrom = bx_options.cdromd.Opresent->get() ? 1 : 0;
        dev_present = dev_is_cdrom || bx_options.diskd.Opresent->get();
        }
      if (!dev_present) {
        continue;
        }
      ...
      if (!dev_is_cdrom) {
        ...
        bx_disk_options *diskopt = (device == 0) ? &bx_options.diskc : &bx_options.diskd;

        int cyl = diskopt->Ocylinders->get ();
        ...
        bool is_vdi_image = false;
        {
          ...
          if (probe_fd >= 0) {
            ...
            long long magic_read = wpb_read(probe_fd, magic, sizeof(magic));
            if (magic_read >= 4) {
              ...
              else if (magic_read >= 8 && memcmp(magic, "conectix", 8) == 0) {
                is_vhd_image = true;
                }
              else if (magic_read >= 37 && memcmp(magic, "<<< Oracle VirtualBox Disk Image >>>", 37) == 0) {
                is_vdi_image = true;
                }
              }
            wpb_close(probe_fd);
        ...
        else if (is_vhd_image) {
          BX_INFO(("HD on ata%d-%d: '%s' 'vpc' mode ", channel, device,
                                  diskopt->Opath->getptr ()));
          channels[channel].drives[device].hard_drive = new vpc_image_t();

          /* open hard drive image file (geometry comes from the vhd footer) */
          if ((BX_HD_THIS channels[channel].drives[device].hard_drive->open(diskopt->Opath->getptr ())) < 0) {
            BX_PANIC(("ata%d-%d: could not open hard drive image file '%s'", channel, device, diskopt->Opath->getptr ()));
            }
          }
        else if (is_vdi_image) {
          BX_INFO(("HD on ata%d-%d: '%s' 'vbox' mode ", channel, device,
                                  diskopt->Opath->getptr ()));
          channels[channel].drives[device].hard_drive = new vbox_image_t();

          /* open hard drive image file (geometry comes from the vdi header) */
          if ((BX_HD_THIS channels[channel].drives[device].hard_drive->open(diskopt->Opath->getptr ())) < 0) {
            BX_PANIC(("ata%d-%d: could not open hard drive image file '%s'", channel, device, diskopt->Opath->getptr ()));
            }
          }
	...
	BX_HD_THIS channels[channel].drives[device].cdrom.cd = new LOWLEVEL_CDROM(bx_options.cdromd.Opath->getptr ());
        BX_INFO(("CD on ata%d-%d: '%s'",channel, device, bx_options.cdromd.Opath->getptr ()));
  ...
  if (!bx_options.cmos.OcmosImage->get ()) {
    ...
    if (BX_DRIVE_IS_HD(0,0)) {
      ...
      DEV_cmos_set_reg(0x1b, (bx_options.diskc.Ocylinders->get () & 0x00ff));
      // AMI BIOS: 1st hard disk #cyl high byte
      DEV_cmos_set_reg(0x1c, (bx_options.diskc.Ocylinders->get () & 0xff00) >> 8);
      // AMI BIOS: 1st hard disk #heads
      DEV_cmos_set_reg(0x1d, (bx_options.diskc.Oheads->get ()));
      ...
      DEV_cmos_set_reg(0x20, (0xc0 | ((bx_options.diskc.Oheads->get () > 8) << 3)));
      ...
      DEV_cmos_set_reg(0x23, bx_options.diskc.Ospt->get ());
    }
    ...
    if (BX_DRIVE_IS_HD(0,1)) {
      ...
      DEV_cmos_set_reg(0x24, (bx_options.diskd.Ocylinders->get () & 0x00ff));
      // AMI BIOS: 2nd hard disk #cyl high byte
      DEV_cmos_set_reg(0x25, (bx_options.diskd.Ocylinders->get () & 0xff00) >> 8);
      // AMI BIOS: 2nd hard disk #heads
      DEV_cmos_set_reg(0x26, (bx_options.diskd.Oheads->get ()));
      ...
      DEV_cmos_set_reg(0x29, (bx_options.diskd.Oheads->get () > 8) ? 0x80 : 0x00);
      ...
      DEV_cmos_set_reg(0x2c, bx_options.diskd.Ospt->get ());
    }
...
(1 |
			      ...
			      case 0xbe: { // read cd
				    if (BX_SELECTED_DRIVE(channel).cdrom.ready) {
					  ...
					  uint32 transfer_length = (BX_SELECTED_CONTROLLER(channel).buffer[6] << 16) |
					                          (BX_SELECTED_CONTROLLER(channel).buffer[7] << 8) |
					                           BX_SELECTED_CONTROLLER(channel).buffer[8];
					  uint8 main_channel_sel = BX_SELECTED_CONTROLLER(channel).buffer[9];
        ...
        case 0xa0: // SEND PACKET (atapi)
	      if (BX_SELECTED_IS_CD(channel)) {
		    ...
		    if ((BX_SELECTED_CONTROLLER(channel).features & (1 << 0)) ||
		        (BX_SELECTED_CONTROLLER(channel).features & (1 << 1))) {
			  BX_ERROR(("PACKET-DMA/overlapped not supported, aborting command"));
...
bx_hard_drive_c::calculate_logical_address(Bit8u channel, off_t *sector)
{
      ...
      if ((Bit64u) logical_sector >= (Bit64u) sector_count) {
...
bx_hard_drive_c::identify_ATAPI_drive(Bit8u channel)
{
  ...
  // No DMA modes supported - PACKET commands are PIO only in this
  // implementation, so don't advertise DMA capability or a guest driver
  // may issue a PACKET with the DMA feature bit set and hit the
  // "PACKET-DMA not supported" panic below.
  BX_SELECTED_DRIVE(channel).id_drive[63] = 0x0000;
...
bx_hard_drive_c::set_cd_media_status(Bit32u handle, unsigned status)
{
  ...
  if (status == 0) {
    ...
    // insert cdrom
#ifdef LOWLEVEL_CDROM
    if (BX_HD_THIS channels[channel].drives[device].cdrom.cd->insert_cdrom(bx_options.cdromd.Opath->getptr())) {
      BX_INFO(( "Media present in CD-ROM drive"));
      BX_HD_THIS channels[channel].drives[device].cdrom.ready = 1;
      BX_HD_THIS channels[channel].drives[device].cdrom.capacity = BX_HD_THIS channels[channel].drives[device].cdrom.cd->capacity();
...
{
      return (off_t)wpb_lseek(fd, offset, whence);
}

ssize_t default_image_t::read (void* buf, size_t count)
{
      return (ssize_t)wpb_read(fd, buf, (long long)count);
}

ssize_t default_image_t::write (const void* buf, size_t count)
{
      return (ssize_t)wpb_write(fd, buf, (long long)count);
}
