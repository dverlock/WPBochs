...
bx_floppy_ctrl_c::init(void)
{
  ...
  BX_FD_THIS s.device_type[0] = bx_options.floppya.Otype->get ();
...
#undef MED
  ...
  BX_FD_THIS s.device_type[1] = bx_options.floppyb.Otype->get ();
...
bx_floppy_ctrl_c::set_media_status(unsigned drive, unsigned status)
{
  ...
  if (status == 0) {
    ...
    if (BX_FD_THIS s.media[drive].fd >= 0) {
      wpb_close( BX_FD_THIS s.media[drive].fd );
...
bx_floppy_ctrl_c::evaluate_media(unsigned type, char *path, floppy_t *media)
{
  ...
  {
    long long fsize = wpb_length(media->fd);
    if (fsize < 0) {
      BX_PANIC(("could not determine floppy image file size for '%s'", path));
      return(0);
      }
    memset(&stat_buf, 0, sizeof(stat_buf));
    stat_buf.st_mode = S_IFREG;
    stat_buf.st_size = (long) fsize;
    ret = 0;
  }
  ...
  if ( S_ISREG(stat_buf.st_mode) ) {
    ...
    switch (type) {
      ...
      case BX_FLOPPY_2_88: // 2.88M 3.5"
        ...
        if ((unsigned long) stat_buf.st_size > (media->sectors * 512)) {
