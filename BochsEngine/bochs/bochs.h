...
#define BX_VGA_LFB_MEM_READ(addr) (bx_devices.pluginVgaDevice->vbe_lfb_mem_read(addr))
#define BX_VGA_LFB_MEM_WRITE(addr, val) bx_devices.pluginVgaDevice->vbe_lfb_mem_write(addr, val)
#if BX_SUPPORT_A20
#  define A20ADDR(x)               ( (x) & bx_pc_system.a20_mask )
#else
#  define A20ADDR(x)               (x)
...
#define BX_IAC()                    bx_devices.pluginPicDevice->IAC()
...
#define BX_LOG_STREAM_STDOUT 0
#define BX_LOG_STREAM_STDERR 1
typedef void (*bx_log_callback_t)(int stream, const char *line);
extern bx_log_callback_t bx_log_callback;
...
typedef struct {
  ...
  bx_param_bool_c   *Ofpu_enabled;
  bx_param_bool_c   *Oprivate_colormap;
  bx_param_bool_c   *Oslowdown_timer_enabled;
#if BX_WITH_AMIGAOS
