...
bx_log_callback_t bx_log_callback = NULL;

void bx_set_log_callback(bx_log_callback_t cb)
{
  bx_log_callback = cb;
}
...
iofunctions::out(int f, int l, const char *prefix, const char *fmt, va_list ap)
{
...

	{
		...
		int n = 0;
		if (prefix != NULL)
			n += _snprintf(line + n, sizeof(line) - n, "%s ", prefix);
		if (l == LOGLEV_PANIC && n < (int)sizeof(line))
			n += _snprintf(line + n, sizeof(line) - n, ">>PANIC<< ");
		if (n < (int)sizeof(line)) {
			va_list ap2;
			va_copy(ap2, ap);
			_vsnprintf(line + n, sizeof(line) - n, fmt, ap2);
			va_end(ap2);
		}
		line[sizeof(line) - 1] = '\0';

		OutputDebugStringD("[Bochs] ");
		OutputDebugStringD(line);
		OutputDebugStringD("\n");
	}

	if (bx_log_callback != NULL) {
		char line[1024];
		int n = 0;
		if (prefix != NULL)
			n += _snprintf(line + n, sizeof(line) - n, "%s ", prefix);
		...
		line[sizeof(line) - 1] = '\0';
		// LOGLEV_ERROR/LOGLEV_PANIC map to the "stderr" stream tag so the
		// UI log popup can render them in red; INFO/DEBUG are "stdout".
		int stream = (l == LOGLEV_ERROR || l == LOGLEV_PANIC)
			? BX_LOG_STREAM_STDERR : BX_LOG_STREAM_STDOUT;
		bx_log_callback(stream, line);
	}
