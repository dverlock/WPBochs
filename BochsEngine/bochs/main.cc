...
bx_options_t bx_options = {
  ...
  NULL,    // default fpu_enabled
  NULL,       // default private_colormap
  NULL,       // default slowdown_timer_enabled
#if BX_WITH_AMIGAOS
...
bx_param_handler (bx_param_c *param, int set, Bit32s val)
{
  ...
  switch (id) {
    ...
    case BXP_CDROM_INSERTED:
      if ((set) && (SIM->get_init_done ())) {
        bx_devices.pluginHardDrive->set_cd_media_status(bx_devices.pluginHardDrive->get_first_cd_handle(), val == BX_INSERTED);
        bx_gui.update_drive_status_buttons ();
      }
      break;
    case BXP_FLOPPYA_STATUS:
      if ((set) && (SIM->get_init_done ())) {
        bx_devices.pluginFloppyDevice->set_media_status(0, val == BX_INSERTED);
        bx_gui.update_drive_status_buttons ();
      }
      break;
    case BXP_FLOPPYB_STATUS:
      if ((set) && (SIM->get_init_done ())) {
        bx_devices.pluginFloppyDevice->set_media_status(1, val == BX_INSERTED);
        bx_gui.update_drive_status_buttons ();
      }
      break;
    case BXP_KBD_PASTE_DELAY:
      if (set && SIM->get_init_done ()) DEV_kbd_paste_delay_changed ();
      break;
    default:
  ...
  switch (param->get_id ()) {
    case BXP_FLOPPYA_PATH:
      if (set==1) {
        if (SIM->get_init_done ()) {
          if (empty) {
            bx_devices.pluginFloppyDevice->set_media_status(0, 0);
            bx_gui.update_drive_status_buttons ();
          } else {
    ...
    case BXP_FLOPPYB_PATH:
      if (set==1) {
        if (SIM->get_init_done ()) {
          if (empty) {
            bx_devices.pluginFloppyDevice->set_media_status(1, 0);
            bx_gui.update_drive_status_buttons ();
          } else {
          ...
          if ((bx_devices.pluginFloppyDevice) &&
            ...
            // tell the device model that we removed, then inserted the disk
            bx_devices.pluginFloppyDevice->set_media_status(1, 0);
            bx_devices.pluginFloppyDevice->set_media_status(1, 1);
          }
        } else {
    ...
    case BXP_CDROM_PATH:
      if (set==1) {
        if (SIM->get_init_done ()) {
          if (empty) {
            bx_devices.pluginHardDrive->set_cd_media_status(bx_devices.pluginHardDrive->get_first_cd_handle(), 0);
            bx_gui.update_drive_status_buttons ();
          } else {
          ...
          if ((bx_devices.pluginHardDrive) &&
              (SIM->get_param_num(BXP_CDROM_INSERTED)->get () == BX_INSERTED)) {
            // tell the device model that we removed, then inserted the cd
            bx_devices.pluginHardDrive->set_cd_media_status(bx_devices.pluginHardDrive->get_first_cd_handle(), 0);
            bx_devices.pluginHardDrive->set_cd_media_status(bx_devices.pluginHardDrive->get_first_cd_handle(), 1);
          }
        } else {
...
void bx_init_options ()
{
  ...
  bx_options.Ofpu_enabled = new bx_param_bool_c (BXP_FPU_ENABLED,
      "Enable the FPU",
      "Controls whether the CPU reports a math coprocessor as present",
      1);
  bx_options.Ofpu_enabled->set_handler (bx_param_handler);
  ...
  bx_options.Oprivate_colormap = new bx_param_bool_c (BXP_PRIVATE_COLORMAP,
      ...
      0);
  bx_options.Oslowdown_timer_enabled = new bx_param_bool_c (BXP_SLOWDOWN_TIMER_ENABLED,
      "Enable the slowdown timer",
      "Throttles emulation speed to match the configured ips against wall-clock time. Disabling lets the emulator run as fast as the device allows.",
      0);
#if BX_WITH_AMIGAOS
  ...
  bx_param_c *interface_init_list[] = {
    ...
    bx_options.Ofpu_enabled,
    bx_options.Oips,
    bx_options.Oprivate_colormap,
    bx_options.Oslowdown_timer_enabled,
#if BX_WITH_AMIGAOS
  ...
  // ACPI options
  bx_options.acpi.Opresent = new bx_param_bool_c (BXP_ACPI_PRESENT,
      "ACPI is present",
      "to be written",
      0);
  bx_options.acpi.Opresent->set (0);
...
parse_line_formatted(char *context, int num_params, char *params[])
{
  ...
  else if (!strcmp(params[0], "fpu")) {
    if (num_params != 2) {
      BX_PANIC(("%s: fpu directive malformed.", context));
      }
    if (strncmp(params[1], "enabled=", 8)) {
      BX_PANIC(("%s: fpu directive malformed.", context));
      }
    if (params[1][8] == '0' || params[1][8] == '1')
      bx_options.Ofpu_enabled->set (params[1][8] - '0');
    else
      BX_PANIC(("%s: fpu directive malformed.", context));
    }
  ...
  else if (!strcmp(params[0], "slowdown_timer")) {
    if (num_params != 2) {
      BX_PANIC(("%s: slowdown_timer directive malformed.", context));
      }
    if (strncmp(params[1], "enabled=", 8)) {
      BX_PANIC(("%s: slowdown_timer directive malformed.", context));
      }
    if (params[1][8] == '0' || params[1][8] == '1')
      bx_options.Oslowdown_timer_enabled->set (params[1][8] - '0');
    else {
      BX_PANIC(("%s: slowdown_timer directive malformed.", context));
      }
    }
  ...
  else if (!strcmp(params[0], "acpi")) {
    for (i=1; i<num_params; i++) {
      if (!strncmp(params[i], "enabled=", 8)) {
	bx_options.acpi.Opresent->set (atol(&params[i][8]));
        }
      }
    }
...
int
bx_write_acpi_options (FILE *fp, bx_acpi_options *opt)
{
  if (!opt->Opresent->get ()) {
    fprintf (fp, "# no acpi\n");
    return 0;
  }
  fprintf (fp, "acpi: enabled=%d\n", opt->Opresent->get ());
  return 0;
}
...
bx_write_configuration (char *rc, int overwrite)
{
  ...
  bx_write_acpi_options (fp, &bx_options.acpi);
  ...
  fprintf (fp, "fpu: enabled=%d\n", bx_options.Ofpu_enabled->get ());
  fprintf (fp, "private_colormap: enabled=%d\n", bx_options.Oprivate_colormap->get ());
  fprintf (fp, "slowdown_timer: enabled=%d\n", bx_options.Oslowdown_timer_enabled->get ());
#if BX_WITH_AMIGAOS
