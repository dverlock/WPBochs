...
public:
  ...
  int    register_timer( void *this_ptr, bx_timer_handler_t, Bit32u useconds,
                         Boolean continuous, Boolean active, const char *id = 0);
  ...
  Bit64u time_usec();
  Bit64u time_usec_sequential() { return time_usec(); }
  unsigned unregisterTimer(int timerID) { UNUSED(timerID); return 0; }
  Bit64u time_ticks();
