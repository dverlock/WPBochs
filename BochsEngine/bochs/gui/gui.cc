...
bx_gui_c::init(int argc, char **argv, unsigned tilewidth, unsigned tileheight)
{
  ...
  // Floppy A:
  BX_GUI_THIS floppyA_status = bx_devices.pluginFloppyDevice->get_media_status(0);
  ...
  // Floppy B:
  BX_GUI_THIS floppyB_status = bx_devices.pluginFloppyDevice->get_media_status(1);
  ...
  BX_GUI_THIS cdromD_status =
    bx_devices.pluginHardDrive->get_cd_media_status(bx_devices.pluginHardDrive->get_first_cd_handle())
...
bx_gui_c::update_drive_status_buttons (void) {
  BX_GUI_THIS floppyA_status = 
    bx_devices.pluginFloppyDevice->get_media_status (0)
    && bx_options.floppya.Oinitial_status->get ();
  BX_GUI_THIS floppyB_status = 
      bx_devices.pluginFloppyDevice->get_media_status (1)
...
bx_gui_c::floppyA_handler(void)
{
  ...
  bx_devices.pluginFloppyDevice->set_media_status(0, BX_GUI_THIS floppyA_status);
...
bx_gui_c::floppyB_handler(void)
{
  ...
  bx_devices.pluginFloppyDevice->set_media_status(1, BX_GUI_THIS floppyB_status);
  BX_GUI_THIS update_drive_status_buttons ();
}
...
{
  BX_GUI_THIS cdromD_status =
    bx_devices.pluginHardDrive->set_cd_media_status(bx_devices.pluginHardDrive->get_first_cd_handle(), !BX_GUI_THIS cdromD_status);
  BX_GUI_THIS update_drive_status_buttons ();
}
...
bx_gui_c::make_text_snapshot (char **snapshot, Bit32u *length)
{
  ...
  bx_devices.pluginVgaDevice->get_text_snapshot(&raw_snap, &txHeight, &txWidth);
...
bx_gui_c::paste_handler(void)
{
  ...
  bx_devices.pluginKeyboard->paste_bytes (bytes, nbytes);
}
