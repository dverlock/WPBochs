...
bx_pc_system_c::dma_write8(Bit32u phy_addr, unsigned channel, Boolean verify)
{
  ...
  UNUSED(channel);
  if (!verify) {
    BX_MEM(0)->writePhysicalPage(BX_CPU(0), phy_addr, 1, &data_byte);
...
bx_pc_system_c::dma_read8(Bit32u phy_addr, unsigned channel)
{
  ...
  Bit8u data_byte;
  UNUSED(channel);
  BX_MEM(0)->readPhysicalPage(BX_CPU(0), phy_addr, 1, &data_byte);
...
bx_pc_system_c::dma_write16(Bit32u phy_addr, unsigned channel, Boolean verify)
{
  ...
  UNUSED(channel);
  if (!verify) {
    BX_MEM(0)->writePhysicalPage(BX_CPU(0), phy_addr, 2, &data_word);
...
bx_pc_system_c::dma_read16(Bit32u phy_addr, unsigned channel)
{
  ...
  Bit16u data_word;
  UNUSED(channel);
  BX_MEM(0)->readPhysicalPage(BX_CPU(0), phy_addr, 2, &data_word);
...
{
  return( bx_devices.pluginPicDevice->IAC() );
}
...
bx_pc_system_c::exit(void)
{
  if (bx_devices.pluginHardDrive)
    bx_devices.pluginHardDrive->close_harddrive();
  BX_INFO(("Last time is %d", (int)bx_devices.pluginCmosDevice->get_timeval()));
  bx_gui.exit();
}
