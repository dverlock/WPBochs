...
// real hardware silently ignores RDMSR/WRMSR to many unimplemented/reserved
#define BxMaxTimerIDLen 32
// MSRs instead of faulting; Windows XP probes several of these during boot,
// so treat unknown MSRs as no-ops instead of raising #GP.
...
#define BX_SOUND_OUTPUT_C  bx_sound_wp_c
#endif
...
// Bochs 2.1 core migration: 2.1's CPU/device code uses bx_bool where
// 1.4.1 used Boolean; keep both available rather than renaming every
// call site across files not yet touched by the migration.
...
#ifdef _MSC_VER
#define BX_C_INLINE static __inline
#else
...
#define BOCHSAPI
#endif
#ifndef BOCHSAPI_CYGONLY
#define BOCHSAPI_CYGONLY
#endif
#define BX_CPP_AttrRegparmN(X) /* not used on MSVC */
#define BX_CPP_AlignN(n) /* not used on MSVC */
#define BX_CPP_AttrPrintf(formatArg, firstArg) /* not used on MSVC */
#define BX_CPP_AttrNoReturn() /* not used on MSVC */

typedef Bit32u bx_address;

#define BailBigRSP(s)
// DEV_pic_iac / DEV_dma_raise_hlda / DEV_cmos_get_reg / DEV_pic_raise_irq /
// DEV_vga_mem_read / DEV_vga_mem_write are defined in plugin.h - don't
// redefine them here (config.h's copies were stale duplicates that just
// got silently shadowed, generating a C4005 warning per translation unit).
...
#define BX_MEM_READ_PHYSICAL(phy_addr, len, ptr) BX_MEM(0)->readPhysicalPage(BX_CPU(0), phy_addr, len, ptr)
#define BX_MEM_WRITE_PHYSICAL(phy_addr, len, ptr) BX_MEM(0)->writePhysicalPage(BX_CPU(0), phy_addr, len, ptr)
